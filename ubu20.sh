...
NAMECOM=$(curl -sS https://raw.githubusercontent.com/boikv2/main/ip | grep $MYIP | awk '{print $2}')
...
FIGHTERTUNNEL() {
    curl -sS https://raw.githubusercontent.com/boikv2/main/ip >/root/tmp
...
ftTunneling() {
    ...
    IZIN=$(curl -sS https://raw.githubusercontent.com/boikv2/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
        TIMEDATE
    else
